...
fwP_params['rewiring_rate'] = [ (0., np.mean([ g[1] for g in fwP_params['rewiring_rate'] ]) ) ]
fwP_params['P'] = [ np.mean([ _P for _P in fwP_params['P'] ]) ]
print fwP_params
...
fig.tight_layout()
fig.savefig('talk/socio_edges_FW_simpleFW.pdf',dpi=300)
...
fig_dt.tight_layout()
i#fig_dt.savefig("talk/distrubution_inter_event_times.pdf")
...
axs[0].plot(x_size, y_size,'s',mfc='None',c=tc.color_sequence[2])
...
for size in range(2,3):
    ...
    axs[2].plot(xgr,ygr,ls='',marker=tc.marker_sequence[size],c=tc.color_sequence[size+2],mfc='None',ms=4)

for i, fig in enumerate(figs):
    fig.tight_layout()
    fig.savefig('talk/socio_'+str(i)+'c.pdf',dpi=300)

t, k = tc.mean_degree(fwP_binned)
ax_k.plot(t/3600.,k,'-',lw=2)
fig_k.tight_layout()
fig_k.tight_layout()
fig_k.savefig('talk/mean_degree_c.pdf')
